...
- **Calendar** — month and list views with category filters.
...
- **Admin dashboard** — create and edit events, post announcements, and see who's registered. Check-in happens here too, with each ticket's QR available for verification.
...
React 19 and TypeScript on Vite, Tailwind v4, and a component library I wrote by hand rather than pulling one in. Supabase covers the database, auth, and realtime. TanStack Query handles server data, Zustand holds the small bits of client state, React Hook Form and Zod take care of forms, and Framer Motion drives the animations.
...
Push the project to GitHub, import it in Vercel with the Vite preset, add `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` as environment variables, and deploy. The rewrite rule in `vercel.json` keeps client-side routing working on refresh.
...
The honest list: email notifications for upcoming events, waitlists when an event fills up, and a dedicated scanner view for the door instead of checking tickets in the admin table. I'd also like to move the registration count on event cards to a realtime subscription so it updates without a refresh.
